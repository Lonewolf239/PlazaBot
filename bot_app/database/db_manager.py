...
class DatabaseInterface:
    ...
    async def get_promoter(self, promoter_id: int):
        data = await self.fetch_one("SELECT * FROM promoters WHERE promoter_id = ?", (promoter_id,))
        if data:
            return data
        await self.execute("INSERT INTO promoters (promoter_id) VALUES (?)", (promoter_id,))
        return None

    async def update_promoter(self, promoter_id: int, last_balance: str):
        await self.get_promoter(promoter_id)
        await self.execute("UPDATE promoters SET last_balance = ? WHERE promoter_id = ?", (last_balance, promoter_id))
