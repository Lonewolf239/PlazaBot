...
class ResourceLoader:
    ...
    @staticmethod
    def load_image_no_square(dir_name: str, image_name: str,
                             size: Optional[tuple[int]] = None) -> Optional[Image.Image]:
        """
        Загружает и масштабирует PNG изображение
    ...
    @staticmethod
    def paste_image_centered_no_square(base_img: Image.Image, overlay_img: Image.Image,
                                       x: int, y: int, width: int, height: int):
        overlay_width, overlay_height = overlay_img.size
        paste_x = x + (width - overlay_width) // 2
        paste_y = y + (height - overlay_height) // 2
        base_img.paste(overlay_img, (paste_x, paste_y), overlay_img)
    ...
    def load_fonts():
        ...
        try:
            return {
                ...
                'very_small': ImageFont.truetype("arial.ttf", 14),
            }
        except IOError:
            try:
                return {
                    'large': ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 34),
                    ...
                    'very_small': ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 14),
                }
            except IOError:
